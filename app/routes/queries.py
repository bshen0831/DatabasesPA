...
def search_awards():
    ...
    query = """ SELECT p.name, mp.name, a.year, COUNT(a.aid) as award_count
    FROM People p JOIN Award a on p.pid = a.pid
    JOIN MotionPicture mp on a.mpid = mp.mpid
    GROUP BY p.name, mp.name, a.year
...
def find_youngest_oldest_actors():
    ...
    query = """ SELECT p.name, (a.award_year - YEAR(p.dob)) as age
    FROM People p JOIN Award a ON p.id = a.pid
    GROUP BY p.name
    HAVING (a.award_year - YEAR(p.dob)) in 
    (
    SELECT MAX(a1.award_year - YEAR(p.dob))
    FROM People p JOIN Award a1 ON p.id = a.pid
    GROUP BY p.id 
    UNION 
    SELECT MIN(a2.award_year - YEAR(p.dob))
    FROM People p JOIN Award a2 ON p.id = a.pid
    GROUP BY p.id
    ); """
...
def search_producers():
    ...
    query = """ SELECT p.name, m.name, m.box_office_collection, m.budget
    FROM People p JOIN Role r on p.pid = r.pid
    JOIN MotionPicture pr on r.pid = pr.pid
    JOIN Movie m on pr.mpid = m.mpid
    WHERE p.country = 'USA' AND m.box_office_collection >= %s AND m.budget <= %s; """
...
def search_multiple_roles():
    ...
    FROM People p JOIN Role r on p.pid = r.pid
    JOIN MotionPicture mp on r.mpid = mp.mpid
    WHERE mp.rating > %s
...
def top_thriller_movies_boston():
    ...
    query = """ SELECT m.name, m.rating 
    FROM MotionPicture mp JOIN Movies m on mp.mpid = m.mpid
    JOIN Location l on mp.mpid = l.mpid
    group by m.name
    having count(l.city) = 1 and l.city = 'Boston' and mp.genre = 'Thriller'"""
...
def search_movies_by_likes():
    ...
    FROM MotionPicture mp JOIN Likes l on mp.mpid = l.mpid
...
def movies_higher_than_comedy_avg():
    ...
    query = """
    Select mp.name, mp.rating FROM MotionPicture mp
    WHERE mp.rating > 
    (SELECT AVG(mp.rating) FROM MotionPicture mp 
    WHERE genre='comedy') 
...
def top_5_movies_people_roles():
    ...
    query = """ SELECT m.name, COUNT(DISTINCT r.pid), COUNT(r.role_name)
...
def actors_with_common_birthday():
    ...
    query = """SELECT p.name, p.name, p.dob
    FROM People p JOIN People p2 ON p.dob = p2.dob AND p.name < p2.name
